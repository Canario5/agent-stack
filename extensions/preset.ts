...
interface Preset {
	...
	tools?: string[];
	/** Tools to remove from the configured or complete tool set */
	excludeTools?: string[];
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		// Apply configured tools, or every registered tool minus exclusions.
		if (preset.tools || preset.excludeTools) {
			const allToolNames = pi.getAllTools().map((t) => t.name);
			const requestedTools = preset.tools ?? allToolNames;
			const validTools = requestedTools.filter((t) => allToolNames.includes(t));
			const invalidTools = requestedTools.filter((t) => !allToolNames.includes(t));
			const excludedTools = new Set(preset.excludeTools ?? []);

			if (invalidTools.length > 0) {
				ctx.ui.notify(`Preset "${name}": Unknown tools: ${invalidTools.join(", ")}`, "warning");
			}

			pi.setActiveTools(validTools.filter((t) => !excludedTools.has(t)));
		}
	...
	function buildPresetDescription(preset: Preset): string {
		...
		if (preset.excludeTools) {
			parts.push(`exclude:${preset.excludeTools.join(",")}`);
		}
